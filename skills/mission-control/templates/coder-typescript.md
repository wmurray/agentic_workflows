...
Spawn with agent type `typescript-developer`, `run_in_background: true`. Fill `{PLACEHOLDERS}`. The org-valued ones (`{MC_HOME}` `{BRANCH_PREFIX}` `{BASE_REF}` `{COMMIT_TRAILER}` `{STYLE_GUIDE}`) come from the overlay's **Template fills** section.
