...
Spawn with agent type `rails-feature-developer`, `run_in_background: true`. Fill `{PLACEHOLDERS}`. The org-valued ones (`{MC_HOME}` `{BRANCH_PREFIX}` `{BASE_REF}` `{WORKTREE_RECIPE}` `{COMMIT_TRAILER}` `{STYLE_GUIDE}` `{TEST_CONVENTIONS}`) come from the overlay's **Template fills** section; `{WORKTREE_RECIPE}` is the overlay's recipe for THIS repo.
